import React from 'react';
import { cn } from '../../utils/cn';
import { useBuiltInWallet } from '../../hooks/useBuiltInWallet';

export interface InputProps extends React.InputHTMLAttributes<HTMLInputElement> {
  label?: string;
  error?: string;
  helperText?: string;
  leftIcon?: React.ReactNode;
  rightIcon?: React.ReactNode;
  fullWidth?: boolean;
}

const Input = React.forwardRef<HTMLInputElement, InputProps>(
  (
    {
      className,
      type = 'text',
      label,
      error,
      helperText,
      leftIcon,
      rightIcon,
      fullWidth = true,
      id,
      ...props
    },
    ref
  ) => {
    const { connectedWallets } = useBuiltInWallet();
    const theme = connectedWallets.length > 0 ? 'cyberpunk' : 'modern';
    const inputId = id || `input-${Math.random().toString(36).substr(2, 9)}`;
    
    return (
      <div className={cn('space-y-2', { 'w-full': fullWidth })}>
        {label && (
          <label
            htmlFor={inputId}
            className={`label text-sm font-medium transition-all duration-300 ${
              theme === 'cyberpunk' 
                ? 'text-white cyberpunk-font' 
                : 'text-secondary-700'
            }`}
          >
            {label}
          </label>
        )}
        
        <div className="relative">
          {leftIcon && (
            <div className={`absolute left-3 top-1/2 transform -translate-y-1/2 transition-all duration-300 ${
              theme === 'cyberpunk' 
                ? 'text-white/60' 
                : 'text-secondary-400'
            }`}>
              {leftIcon}
            </div>
          )}
          
          <input
            id={inputId}
            type={type}
            className={cn(
              'input transition-all duration-300',
              theme === 'cyberpunk' 
                ? 'cyberpunk-input' 
                : '',
              {
                'pl-10': leftIcon,
                'pr-10': rightIcon,
                'border-error-300 focus:ring-error-500 focus:border-error-500': error && theme !== 'cyberpunk',
                'border-secondary-300 focus:ring-primary-500 focus:border-primary-500': !error && theme !== 'cyberpunk',
              },
              className
            )}
            ref={ref}
            {...props}
          />
          
          {rightIcon && (
            <div className={`absolute right-3 top-1/2 transform -translate-y-1/2 transition-all duration-300 ${
              theme === 'cyberpunk' 
                ? 'text-white/60' 
                : 'text-secondary-400'
            }`}>
              {rightIcon}
            </div>
          )}
        </div>
        
        {error && (
          <p className={`text-sm transition-all duration-300 ${
            theme === 'cyberpunk' 
              ? 'text-red-400' 
              : 'text-error-600'
          }`}>{error}</p>
        )}
        
        {helperText && !error && (
          <p className={`text-sm transition-all duration-300 ${
            theme === 'cyberpunk' 
              ? 'text-white/60' 
              : 'text-secondary-500'
          }`}>{helperText}</p>
        )}
      </div>
    );
  }
);

Input.displayName = 'Input';

export { Input };
